...
{
    public VR_ControllerTooltips.TooltipButtons element;
}
...
public class VR_ControllerTooltips : MonoBehaviour
{
    ...
    protected VR_ObjectTooltip[] buttonTooltips = new VR_ObjectTooltip[0];
    ...
    protected virtual void InitButtonsArray()
    {
        ...
        buttonTooltips = new VR_ObjectTooltip[availableButtons.Length];
        tooltipStates = new bool[availableButtons.Length];

        for (int i = 1; i < availableButtons.Length; i++)
        {
            buttonTooltips[i] = transform.Find(availableButtons[i].ToString()).GetComponent<VR_ObjectTooltip>();
        }
    }
    ...
    {
        VR_ObjectTooltip[] tooltips = GetComponentsInChildren<VR_ObjectTooltip>(true);
        for (int i = 0; i < tooltips.Length; i++)
        {
            VR_ObjectTooltip tooltip = tooltips[i];
            string tipText = "";
            ...
            switch (tooltip.name.Replace("Tooltip", "").ToLower())
            {
                ...
                case "grip":
                    tipText = gripText;
                    tipTransform = GetTransform(grip);
                    break;
                case "touchpad":
                    tipText = touchpadText;
                    tipTransform = GetTransform(touchpad);
                    break;
                case "buttonone":
                    tipText = buttonOneText;
                    tipTransform = GetTransform(buttonOne);
                    break;
                case "buttontwo":
                    tipText = buttonTwoText;
                    tipTransform = GetTransform(buttonTwo);
                    break;
            }
    ...
    protected virtual Transform GetTransform(Transform setTransform)
    {
