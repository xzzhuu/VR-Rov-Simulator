...
public class VolumetricProbe : MonoBehaviour
{
    ...
    public enum IslandTypes { Empty, Duplicate, Rect, Circle };
    ...
    private void Update()
    {
        ...
        if (oldLightPos != lightSource.transform.position || oldLightRot != lightSource.transform.eulerAngles || oldPos != transform.position || oldRot != transform.eulerAngles || lightSource.color != oldLightColor || lightSource.intensity != oldLightIntensity)
            UpdateMesh();
    }
    ...
    public void UpdateMesh()
    {
        ...
        oldLightColor = lightSource.color;
        oldLightIntensity = lightSource.intensity;
        ...
        for (int s = 0; s < islands.Length; s++)
        {
            ...
            {
                Color col = useLightColor ? lightSource.color * startColor : startColor;
                if (useLightIntensity)
                    col *= Mathf.Clamp(lightSource.intensity, 0, lightIntensityClamp);
                vertColor.Add(col);
            }
            ...
            for (int i = 0; i < islands[s].vertices.Length; i++)
            {
                ...
                Color color = useLightColor ? lightSource.color : Color.white;
                if (useLightIntensity)
                    color *= Mathf.Clamp(lightSource.intensity, 0, lightIntensityClamp);
                color.a = 0;
                RaycastHit hit;
                Vector3 dir;
